...
for i in [PXD000923,PXD002222,PXD002756,PXD004705,PXD004939,PXD005241,PXD012764,PXD019291]:
    ...
    df = df.rename(columns={'All_Proteins':'Proteins','Peptide':'Unmodified Sequence','Score':'PSM Score','PTM Score':'Site probability','All_USI':'Supporting PSM count',
                            'PTM positions':'Site position','Binomial_final_score':'Final adjusted site probability','Peptide_mod':'Modified Peptide','All_PTMs':'Modifications','All_PTM_scores':'Modification probabilities',
    ...
    df=df[~df['Modified Peptide'].str.contains("A\[Phosphorylation\]")==True]
    ######
    ...
    #If pos=-1, no choice peptide, delete row
    df=df.loc[df['Site position']>0]
    ...
    df = df.sort_values(by=(['Pep_pos','Modified Peptide','Final adjusted site probability']), ascending=[True, True, True])
    ...
    df['Site Passes Threshold']=np.where(df['Binomial final adjusted q_value'] <= FLR_threshold, 1, 0)
    ...
    df = df[['Site ID', 'Proteins', 'Unmodified Sequence', 'Modified Peptide','Modification','Site position', 'Site probability', 'PSM Score',
            'Final adjusted site probability','Binomial final adjusted q_value','Site Passes Threshold', 'Supporting PSM count','Supporting PSM Count (Thresholded 0.05 Global FLR)', 'Dataset ID', 'PRIDE dataset URL', 'PubMedID',
             'Sample ID', 'USI','PRIDE dataset URL (including the USI)','PSM count 0.01<P<=0.05','PSM count 0.05<P<=0.19','PSM count 0.19<P<=0.81','PSM count 0.81<P<=0.95','PSM count 0.95<P<0.99','PSM count P>=0.99']]
    df.to_csv("C:/Users/krams/Dropbox/PTMExchange/Rice/New_build_ID/Site_format_files/"+dataset_ID[0]+"_Site_peptidoform_centric_binomial_FLR_no_pA_bins_"+str(FLR_threshold)+"_updated.tsv", sep="\t", index=False)
