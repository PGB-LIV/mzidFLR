...
#TPP_comparison.py [mzid_file] [PXD] [mod] [optional: FDR_cutoff]
if len(sys.argv)<3 or len(sys.argv)>5:
    sys.exit("Provide mzid file, PXD identifier and modification of interest. TPP_comparison.py [mzid_file] [PXD] [modification:target:decoy] [optional: FDR_cutoff] [optional: PTM_score_cutoff]")
mzid_file=sys.argv[1]
if mzid_file[-5:]!=".mzid":
    print(mzid_file[-5:])
    sys.exit("Provide mzid file. TPP_comparison.py [mzid_file] [PXD] [modification:target:decoy]] [optional: FDR_cutoff] [optional: PTM_score_cutoff]")
PXD = sys.argv[2]
if "PXD" not in PXD:
    sys.exit("Provide PXD identifier. TPP_comparison.py [mzid_file] [modification:target:decoy]] [PXD] [optional: FDR_cutoff] [optional: PTM_score_cutoff]")
mod_info = sys.argv[3]
if len(mod_info.split(":"))!=3:
	sys.exit("Provide modification in format modification:target:decoy. E.g. Phospho:STY:A. TPP_comparison.py [mzid_file] [modification:target:decoy]] [PXD] [optional: FDR_cutoff] [optional: PTM_score_cutoff]")
try:
